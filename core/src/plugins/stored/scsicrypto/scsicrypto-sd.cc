...
static bRC do_set_scsi_encryption_key(void* value)
{
  ...
  *StoredVolEncrKey = '\0';
  if (!GetVolumeEncryptionKey(dcr, StoredVolEncrKey)) {
    ...
    if (!NeedScsiCryptoKey(dev->fd_, dev->dev_name, true)) {
  ...
  P(crypto_operation_mutex);
  if (SetScsiEncryptionKey(dev->fd_, dev->dev_name, VolEncrKey)) {
...
static bRC do_clear_scsi_encryption_key(void* value)
{
  ...
  if (device_resource->query_crypto_status) {
    need_to_clear = IsScsiEncryptionEnabled(dev->fd_, dev->dev_name);
  } else {
    need_to_clear = dev->IsCryptoEnabled();
  }
  if (need_to_clear) {
    Dmsg0(debuglevel, "scsicrypto-sd: Clearing crypto key\n");
    if (ClearScsiEncryptionKey(dev->fd_, dev->dev_name)) {
  ...
  if (device_resource->drive_crypto_enabled) {
    ...
    switch (dev->dev_errno) {
      case EIO:
        ...
        if (device_resource->query_crypto_status) {
          P(crypto_operation_mutex);
          if (NeedScsiCryptoKey(dev->fd_, dev->dev_name, false)) {
        ...
        if (decryption_needed) {
          ...
          Mmsg5(
              ...
              dev->fd_, dev->file, dev->block_num, dev->print_name(),
              be.bstrerror());
        }
        break;
      default:
...
static bRC send_device_encryption_status(void* value)
{
  ...
  if (dst->device_resource->drive_crypto_enabled) {
    ...
    dst->status_length = GetScsiDriveEncryptionStatus(
        dst->device_resource->dev->fd_, dst->device_resource->dev->dev_name,
        dst->status, 4);
    V(crypto_operation_mutex);
  }
  return bRC_OK;
